...
volatile bool worCycleRequested = false;
...
void setupLoRa() {
  ...
  Serial.print(F("[SX126x] Initializing Hub Tx... "));
...
void sendOutsideWakeRequest() {
  ...
  if (state == RADIOLIB_ERR_NONE) {
    Serial.println(F("[LoRa] WOR packet sent OK"));
    radio.standby();
  } else {
    Serial.printf("[LoRa] WOR send failed, code %d\n", state);
  }
  // no flag side effect here anymore
}
...
void sendTenWakeRequests() {
  ...
  Serial.println();
  Serial.println(F("========================================"));
  Serial.println(F("[LoRa] Starting 10 WOR transmissions"));
  Serial.println(F("========================================"));

  // 1-byte throw-away WOR payload
  uint8_t payload = 0x55;

  for (int i = 0; i < NUM_WOR_TX; i++) {
...
void setup() {
  ...
  server.on("/setpoint", HTTP_GET, []() {
    if (server.hasArg("value")) {
      ...
      if (newSetpoint >= 40.0 && newSetpoint <= 99.0) {  // sanity clamp
        thermostatSetpoint = newSetpoint;
        saveState();  // extend saveState()/restoreState() to persist thermostatSetpoint too
        server.send(200, "text/plain", "OK: setpoint=" + String(thermostatSetpoint, 1));
      } else {
        server.send(400, "text/plain", "Out of range");
      }
    } else {
      server.send(200, "text/plain", "Current setpoint: " + String(thermostatSetpoint, 1));
    }
  });
  server.begin();
...
void logData() {
  ...
  appendLog.print(" F, Inside: ");
  appendLog.print(sensordata.insideTemp, 2);
  appendLog.print(" F, In.Humidity: ");
  appendLog.print(sensordata.insideHumidity, 2);
  appendLog.print(" %, Thermostat: ");
  appendLog.print(thermostatSetpoint, 2);
  appendLog.print(" F, Event: ");
  appendLog.print(sensordata.lastEventMinutes, 2);
  appendLog.print(", Total 24HR: ");
  appendLog.print(sensordata.dailyTotalMinutes, 2);
  appendLog.print(", OutP: ");
  if (bmeOffline) appendLog.print("Offline");
  else appendLog.print(toSeaLevelHPa(sensordata.outsidePressure, sensordata.outsideTemp) * 0.02953, 4);
  appendLog.print(" inHg, InP: ");
  appendLog.print(toSeaLevelHPa(sensordata.insidePressure, sensordata.insideTemp) * 0.02953, 4);
  appendLog.print(" inHg, Diff: ");
  if (bmeOffline) appendLog.print("Offline");
  else appendLog.print(sensordata.pressureDiffHPa * 0.02953, 4);
  appendLog.print(" inHg, Cycles: ");
  appendLog.print(cyclesToday);
  appendLog.print(", Coast: ");
  appendLog.print(lastCoastMinutes, 1);
  appendLog.print(" min, AvgCycle: ");
  appendLog.print(avgCycleMinutes, 2);
  appendLog.print(" min\n");
  appendLog.close();
}
